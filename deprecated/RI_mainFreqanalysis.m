...
cfg.pad             = 1000;                                                 % data are padded with zero pads to a length of 4*fs
cfg.foi             = 2:1:30;                                               % analysis from 1 to 30 Hz in steps of 1 Hz 
cfg.t_ftimwin       = ones(length(cfg.foi),1) .* 1;
cfg.toi             = 'all';                                                % spectral estimates on each sample
%cfg.feedback        = 'no';                                                % suppress feedback output
%cfg.showcallinfo    = 'no';                                                % suppress function call output

data_fft = ft_freqanalysis(cfg, data_head_ext{1});                          % calculate time frequency responses
...
data_wav = ft_freqanalysis(cfg, data_head_ext{1});                          % calculate time frequency responses
